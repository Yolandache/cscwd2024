...
import os
import pandas as pd
from FeatureExtra import global_path
...
def tcp_tuple(path):
    ...
    获取所有pcap文件内的四元组数据
    '''
    # files = ['192.168.21.82.pcap']
    for file in files:
        """
        ['ip.src', 'ip.dst',               'in_packets', 'in_bytes', 'out_packets', 'out_bytes', 'duration']
        192.168.0.178:1036  <-> 23.44.53.185:443     4295 6494 kB             2303 138 kB      6598 6633 kB      9.188156000       26.4608
        """
        pcap = path + '\\' + file
        data = os.popen(cmd.format(pcap=pcap)).read()   #
        data = np.array(data.split('|')[-1].split('=')[0].split())  #切割出需要的数据内容
        df = pd.DataFrame(data.reshape(-1, 14)) #重新排列，转为csv格式
        ...
        # print(df.iloc[0:2, :])
        # x = input('..')
        df = df.drop([1, 5, 8, 9, 10, 11, 12], axis=1)  #筛除无用数据
...
if __name__ == '__main__':
    tuple_data_path = global_path.csv_path + '\\content.csv'
    print(tuple_data_path,os.path.exists(tuple_data_path))
